...
## 🏗 Core Engineering Philosophy (Original)
...
- **Issue tracker**: The tool that hosts a repo's issues — GitHub Issues, Linear, a local `.scratch/` markdown convention, or similar. (_Avoid_: backlog manager, backlog backend, issue host)
- **Issue**: A single tracked unit of work inside an **Issue tracker** — a bug, task, PRD, or slice produced by `to-issues`. (_Avoid_: ticket)
- **Triage role**: A canonical state-machine label applied to an **Issue** during triage (e.g. `needs-triage`, `ready-for-afk`).
...
### Flagged ambiguities
- **"backlog"**: Resolved: the tool is the **Issue tracker**; "backlog" is no longer used as a domain term.
...
- **Grilling**: The Socratic process of alignment before execution, typically resulting in a PRD. In this port, it is enhanced with **Empirical Protocols** (searching before asking).
